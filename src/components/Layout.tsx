...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed lg:static lg:relative lg:h-screen inset-y-0 left-0 z-40 w-64 bg-indigo-900 text-white transition-transform duration-300 ease-in-out lg:flex lg:flex-col
      ...
      `}>
        ...
        <div className="flex items-center gap-3 px-4 py-3 mb-4 lg:hidden">
          ...
          <button
            ...
            className="ml-auto text-sm text-red-300 bg-red-900/10 px-3 py-1 rounded-lg hover:bg-red-900/20"
          >
            <LogOut size={16} />
        ...
        <nav className="mt-6 px-4 space-y-2 lg:flex-1 lg:overflow-y-auto relative z-20">
          {menuItems.map((item) => {
        ...
        <div className="hidden lg:block lg:mt-auto w-full px-4 py-0 border-t border-indigo-800 z-10">
          ...
          <button 
            onClick={() => { onLogout(); navigate('/login'); }}
            className="flex items-center gap-3 w-full px-4 py-2 rounded-lg transition-colors hover:bg-red-900/30 text-red-300"
          >
            <LogOut size={20} />
            <span>Logout</span>
          </button>
