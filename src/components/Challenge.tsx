...
import { createSubmission, fetchChallengeById, getCurrentUser } from '../api';
...
export const ChallengeDetail: React.FC = () => {
  ...
  if (loadingChallenge) {
    return (
      <div className="flex items-center justify-center py-20 text-slate-500">Loading challenge details…</div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createSubmission({ challengeId: id, githubLink: submissionLink, language }, currentUser.id);
      setSubmitted(true);
    } catch (err) {
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <label className="block text-sm text-slate-500">
                    ...
                    <input
                      ...
                      className="mt-2 w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                    />
                  </label>
                </div>
