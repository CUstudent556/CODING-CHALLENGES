...
import { MOCK_SUBMISSIONS, MOCK_CHALLENGES } from '../mockData';
      ...
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {filtered.map((sub) => {
              const challenge = MOCK_CHALLENGES.find(c => c.id === sub.challengeId);
              return (
                <tr key={sub.id} className="hover:bg-slate-50">
                  <td className="px-6 py-5 font-medium text-slate-900">{challenge?.title}</td>
